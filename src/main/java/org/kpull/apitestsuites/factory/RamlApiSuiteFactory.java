    ...
    public ApiSuite generate() {
        ...
        ramlModel.getResources().entrySet().stream().forEach(resource -> {
            ...
            resourceContent.getActions().entrySet().forEach(action -> {
                ...
                ApiCall apiCall = new ApiCall(name, StringUtils.defaultString(actionContent.getDescription()), new ApiRequest(actionType.name(), resourceContent.getUri(), Collections.emptyList(), "application/json", "", Collections.emptyList()), new ApiResponse(Collections.emptyList(), 0, "", ""), "");
